...
pub fn execute_transfer(
    ...
    // 0.05 tax burned from every transfer.
    // TODO: Adjust tax based on decimals, currently hardcoded for 8.
    let tax_amount = amount.checked_mul(Uint128::from(5u128)).unwrap().checked_div(Uint128::from(100u128)).unwrap();
    let recieve_amount = amount.checked_sub(tax_amount).unwrap();

    deps.api.debug(&tax_amount.to_string());
    deps.api.debug(&recieve_amount.to_string());

    BALANCES.update(
        deps.storage,
        &rcpt_addr,
        |balance: Option<Uint128>| -> StdResult<_> { Ok(balance.unwrap_or_default() + recieve_amount) },
    )?;

    // reduce total_supply
    TOKEN_INFO.update(deps.storage, |mut info| -> StdResult<_> {
        info.total_supply = info.total_supply.checked_sub(tax_amount)?;
        Ok(info)
    })?;
    ...
    let res = Response::new()
        ...
        .add_attribute("amount", recieve_amount);
    Ok(res)
}
...
mod tests {
    ...
    #[test]
    fn transfer() {
        let mut deps = mock_dependencies(&coins(8, "token"));
        let addr1 = String::from("addr0001");
        let addr2 = String::from("addr0002");
        let amount1 = Uint128::from(12340000u128);
        let amount1_minux_tax = Uint128::from(12336173u128);
        let transfer = Uint128::from(76543u128);
        let transfer_minux_tax = Uint128::from(72716u128);
        ...
        assert_eq!(get_balance(deps.as_ref(), addr2), transfer_minux_tax);
        assert_eq!(
